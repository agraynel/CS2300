	<!--
	CREDITS: All of the photos and background are from my roommate, Chuan Huang
      I downloaded from his loft: http://akimotoyasushi.lofter.com/
      Others were sent to me from him by e-mail
    CREDITS: index.php modified from index.php in my project2
    -->
<!DOCTYPE html>

<html>

	<head>
		<meta charset="utf-8">
    	<link rel="stylesheet" type="text/css" href="css/stylesheet.css">
    	<meta name="viewport" content="width=device-width, initial-scale=1">
    	<script src = "js/validation.js"></script>
		<title>H.c photography</title>
	</head>

	<body>
<!--
  This is Yi Chen(yc2329)'s work for CS 2300 project 3 milestone1.
  This is a personal photography web[age for my roommate, Chuan Huang (Cornell Vet School Ph.D)
	CREDITS: All of the photos are from my roommate, Chuan Huang
      I downloaded from his loft: http://akimotoyasushi.lofter.com/
      Others were sent to me from him by e-mail
  CREDITS: this page is modified from header.php in my project 2.
-->
 
    <!-- Navigation Bar -->
    <div id='navigation'>
      <a title="home" href="index.php"> HOME</a>
      <a title="gallery" href="files/gallery.php"> GALLERY</a>
      <a title="login" href="files/login.php"> LOGIN</a>
      <a title="add" href="files/add.php"> ADD</a>
      <a title="search" href="files/search.php"> SEARCH</a>
    </div>  
    <div id='divider'></div>
    <h5>CREDITS: All images from Chuan Huang.</h5>
	
	<!-- Welcome message -->
  <h1>H.c Photography</h1>
	<img class="image_origin" src="assets/background.jpg" alt="index_background" />
	</body>
</html>